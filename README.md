...
## Important OAuth architecture

Notion's public OAuth flow requires a **client secret** when the authorization code is exchanged for an access token. That secret must not be embedded in a Chrome extension. Therefore this project has two pieces:

1. `extension/` — the Chrome extension. It captures tabs, presents the workspace/database picker, and calls the Notion REST API.
2. `backend/cloudflare-worker/` — a tiny OAuth helper. It holds the Notion client secret and performs only OAuth code exchange/token refresh.

The extension never sees your Notion password. Authentication happens on Notion's own OAuth page.
...
2. For a personal/private deployment, **Selected workspaces only** is appropriate; choose the workspace(s) you want to connect. If you later intend to distribute Tabs2Notion broadly, use the appropriate public installation scope instead.
...
During authorization, Notion presents its page picker. Grant Tabs2Notion access to the databases you want to appear in the destination selector, or to parent pages containing those databases.

## 2. Deploy the OAuth Worker

The backend is intentionally small and does not store your tokens in a database. The access and refresh tokens are returned to the extension through Chrome's OAuth redirect and stored in `chrome.storage.local` for that extension profile.
...
For `STATE_SECRET`, use a long random value, for example one generated by a password manager or:
...
6. Paste your Worker base URL, e.g. `https://tabs2notion-oauth.<your-subdomain>.workers.dev`.
...
Notion currently rate-limits integrations to an average of roughly 3 requests/second per connection (with an additional workspace-level limit). Tabs2Notion saves pages sequentially with spacing between creates and respects `Retry-After` on HTTP 429/529 responses. This makes a 50-tab dump slower than a local bookmark operation, but substantially more reliable.
...
- OAuth tokens are stored in `chrome.storage.local`; they are not intentionally synced through Chrome Sync.
...
`0.1.0` — functional first implementation intended for an unpacked/private Chrome deployment. Before publishing in the Chrome Web Store, add a privacy policy, finalize branding/store assets, review permission disclosures, and use a stable production OAuth backend URL.
